...
function pagination ($contenu, $articlesPerPage) {
	$articles = count($contenu);
	$nomPages = ceil($articles/$articlesPerPage);
...
function displayArticles($contenu, $articlesPerPage) {

	$articles = count($contenu);
	$nomPages = ceil($articles/$articlesPerPage);

	$start = 1;
	while ($start <= $nomPages) {
		if (isset($_GET["$start"])) {
			$firstArticle = (($start-1)*$articlesPerPage)+1;
			while ($firstArticle <= $start*$articlesPerPage) {
				if (empty($contenu[$firstArticle])) {
					echo "";
				}

				else {
					echo "<h1>".$contenu[$firstArticle]["titre"]."</h1>";
					echo "<p>".$contenu[$firstArticle]["letexte"]."</p>";
					echo "<p class='date'>".$contenu[$firstArticle]["ladate"]."</p>";
					echo "<hr>";
				}				
				$firstArticle++;
			}
